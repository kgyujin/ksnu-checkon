...
> **"공정위 데이터를 기준점(Anchor)으로 삼아 국세청의 최신 상태값과 특허청의 자산 정보를 교차 검증함으로써, 데이터의 정합성을 확보한 신뢰도 분석 리포트를 제공합니다."**

여러 공공 데이터를 한 번에 조회하여 사업자 정보를 종합적으로 확인할 수 있는 도구입니다.

## 🎯 프로젝트 목표

**정보 비대칭 해소**: 사용자가 일일이 3개 사이트(홈택스, 공정위, 키프리스)를 돌아다녀야 하는 수고를 **"사업자번호 입력 1회"**로 줄였습니다.

**기술적 차별성**: 기존 서비스들이 단순히 데이터를 나열하는 데 그쳤다면, CheckOn은:
- ✅ **2차 필터링**: KIPRIS API 응답을 출원인 일치 여부로 검증 (False Positive 방지)
- ✅ **3단계 교차 검증**: 실존성(국세청) + 투명성(공정위) + 지속성(운영기간/IP)
- ✅ **객관적 근거 제시**: 추상적 '신뢰도 점수' 대신 구체적 '검증 배지' 제공
...
**'신뢰도 점수' 대신 '검증(Verified)' 배지로 객관적 근거 제시**

#### ✅ 검증 1: 실존성 (국세청)
- **목적**: 사업자의 실제 존재 여부 확인
- **기준**: 국세청 '계속사업자' 상태 확인
- **검증**: 사업자등록번호 유효성 + 정상 운영 상태
- **결과**: 계속사업자 = 실존성 검증 완료

#### ✅ 검증 2: 투명성 (공정위)
- **목적**: 사업 투명성 및 신뢰성 확인
- **기준**: 공정위 통신판매업 신고 + 공식 웹사이트 운영
- **검증**: 신고번호 존재 + 도메인 정보 확인
- **결과**: 공정위 등록 + 웹사이트 = 투명성 검증 완료

#### ✅ 검증 3: 지속성 (사업영위기간 OR 지식재산권)
- **목적**: 사업의 지속 가능성 확인
- **기준 A**: 개업일로부터 **5년 이상** 운영
- **기준 B**: 지식재산권(상표/특허) 보유 + **출원인 일치**
- **검증**: 
  - 개업일 계산: `calculateOperatingYears(startDt)`
  - IP 보유: `trademarkResult.status === 'OK' || patentResult.status === 'OK'`
  - **2차 필터링**: `extractAllXmlElements()` + `isApplicantMatching()` 로 출원인 검증
- **결과**: 5년+ OR IP보유 = 지속성 검증 완료

**종합 검증 결과 예시:**
- 3단계 모두 통과: ✅ "국세청과 공정위에 정상 등록, 5년 이상 운영 중인 신뢰할 수 있는 기업"
- 2단계 통과: ⚠️ "기본 신뢰 요소 확인, 거래 전 추가 확인 권장"
- 1단계만 통과: ❓ "신뢰도 제한적, 충분한 검증 필요"
- 모두 미통과: ⛔ "신뢰도 확인 불가"
